"""全体最適化 v4 の Optuna storage から図と表を生成する。"""
...
STORAGE_PATH = Path("data") / "optuna" / "whole_plant_optuna_v4.db"
MEDIA_DIR = Path("scripts") / "whole_plant_optuna_v4" / "media"
RESULTS_DIR = Path("scripts") / "whole_plant_optuna_v4" / "results"
STUDY_NAMES: tuple[str, ...] = (
    "radial_2stage_whole_plant_profit_v4",
    "radial_3stage_whole_plant_profit_v4",
...
def plot_whole_plant_optuna_v4_main() -> None:
    ...
    plot_best_objective_history(
        studies=studies, output_path=MEDIA_DIR / "best_objective_history.png"
    )
    ...
    for study in studies:
        ...
        for trial in sorted(trials, key=lambda item: item.number):
            if trial.value is None:
                continue
            best_value = (
                trial.value if best_value is None else max(best_value, trial.value)
            )
...
def plot_objective_trials(studies: tuple[optuna.Study, ...], output_path: Path) -> None:
    ...
    for study in studies:
        trials = complete_trials(study)
        values = [
            (trial.number, trial.value) for trial in trials if trial.value is not None
        ]
        if not values:
            continue
        ax.scatter(
...
def write_top_trials(studies: tuple[optuna.Study, ...]) -> None:
    ...
    all_summaries = tuple(
        (summary for study in studies for summary in summaries_from_study(study)),
    )
    summaries = sorted(
        all_summaries,
    ...
    write_top_trials_csv(
        summaries=summaries, output_path=RESULTS_DIR / "top_trials.csv"
    )
    write_top_trials_markdown(
        summaries=summaries, output_path=RESULTS_DIR / "top_trials.md"
    )
...
def write_top_trials_markdown(summaries: list[TrialSummary], output_path: Path) -> None:
    ...
    output_path.write_text(markdown_table(summaries=summaries) + "\n", encoding="utf-8")


def write_results_markdown(summaries: tuple[TrialSummary, ...], output_path: Path) -> None:
    """2段、3段ごとの上位 trial Markdown を保存する。"""
    overall_summaries = sorted(
        summaries,
        key=lambda item: item.objective_yen_per_year,
        reverse=True,
    )[:TOP_TRIAL_COUNT]
    lines = [
        "# Whole Plant Optuna v4 Results",
        "",
        "## Overall Top Trials",
        "",
        markdown_table(summaries=overall_summaries),
        "",
    ]
    for stage_count in (2, 3):
        stage_summaries = sorted(
            (
                ...
                if summary.reactor_type == "radial" and summary.stage_count == stage_count
            ),
            key=lambda item: item.objective_yen_per_year,
            reverse=True,
        )[:TOP_TRIAL_COUNT]
        lines.extend(
            [
                f"## Radial {stage_count} Stage Top Trials",
                "",
                markdown_table(summaries=stage_summaries),
                "",
            ]
        )
...
def markdown_table(summaries: list[TrialSummary]) -> str:
    """上位 trial の Markdown 表を返す。"""
...
def top_trial_columns() -> list[str]:
    ...
    return [
        ...
        "bed_thicknesses_m",
        "bed_heights_m",
        "inlet_velocities_m_per_s",
        "outlet_velocities_m_per_s",
        "ld_ratios",
        "eb_conversion",
        "styrene_selectivity",
        "fresh_eb_kmol_h",
...
def top_trial_row(rank: int, summary: TrialSummary) -> dict[str, object]:
    ...
    return {
        ...
        "decanter_1_temperature_c": format_attr(summary.attrs, "decanter_1_temperature_c"),
        "sm_column_reflux_ratio": format_attr(summary.attrs, "sm_column_reflux_ratio"),
        ...
        "bed_heights_m": join_stage_values(summary.attrs, "bed_height_m"),
        "inlet_velocities_m_per_s": join_stage_values(
            summary.attrs, "inlet_velocity_m_per_s"
        ),
        "outlet_velocities_m_per_s": join_stage_values(
            summary.attrs, "outlet_velocity_m_per_s"
        ),
        ...
        "annualized_equipment_yen_per_year": format_attr(
            summary.attrs, "annualized_equipment_yen_per_year"
        ),
    }
...
if __name__ == "__main__":
    plot_whole_plant_optuna_v4_main()
