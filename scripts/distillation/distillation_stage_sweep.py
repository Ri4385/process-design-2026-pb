...
def utility_cost_yen_per_year(flowsheet: Any, target_tower: TargetTower) -> float:
    """対象塔に対応する用役費を計算する。"""
...
def evaluate_feed_stage(
    ...
    try:
        ...
        refs = fresh_column_refs(simulation_case, target_tower)
        utility = utility_cost_yen_per_year(refs.flowsheet, target_tower)
        objective = equipment_cost_yen_per_year + utility
        ...
        return FeedStageResult(
            ...
            utility_cost_yen_per_year=utility if valid else None,
            objective_yen_per_year=objective if valid else None,
...
def write_figure(target_tower: TargetTower, results: Sequence[CaseSweepResult]) -> Path:
    ...
    figure_path = MEDIA_DIR / f"{target_tower}_cost_summary.png"
...
def print_summary(
    ...
    if not candidates:
        log(f"[done] tower={target_tower} no valid case figure={figure_path}")
        return

    best = min(candidates, key=lambda result: result.objective_yen_per_year or math.inf)
    log(
        f"[done] tower={target_tower} best_N={best.stage_count} "
        f"best_feed={best.best_feed_stage} "
        f"best_J={cost_to_oku_yen_per_year(best.objective_yen_per_year):.4f} "
...
def main() -> None:
    ...
    log(f"[start] tower={TARGET_TOWER} grid={GRID_LEVEL} cases={len(case_paths)}")
