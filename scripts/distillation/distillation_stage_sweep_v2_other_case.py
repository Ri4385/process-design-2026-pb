...
TOWER2_MAX_BOTTOM_TEMPERATURE_C = 100.0
...
def tower2_constraint_reason(
    heat_exchanger: ColumnHeatExchangerCost,
    target_tower: TargetTower,
) -> str:
    """tower2 の塔底温度制約違反理由を返す。"""
    if target_tower != "tower2":
        return ""
    if heat_exchanger.bottom_temperature_c <= TOWER2_MAX_BOTTOM_TEMPERATURE_C:
        return ""
    return (
        f"tower2 bottom temperature {heat_exchanger.bottom_temperature_c:.3f} C "
        f"> {TOWER2_MAX_BOTTOM_TEMPERATURE_C:.1f} C"
    )
...
def evaluate_feed_stage(
    ...
    try:
        ...
        constraint_reason = tower2_constraint_reason(heat_exchanger, target_tower)
...
def feed_result_payload(result: FeedStageResult) -> dict[str, Any]:
    """FeedStageResult を JSON 保存用 dict に変換する。"""
    return result.model_dump(mode="json")


def write_direction_results(
    output_path: Path,
    status: str,
    results: Sequence[FeedStageResult],
    error: str = "",
) -> None:
    """direction 子プロセスの途中結果を JSON に保存する。"""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "status": status,
        "error": error,
        "results": [feed_result_payload(result) for result in results],
    }
    with output_path.open("w", encoding="utf-8") as file:
        json.dump(payload, file, ensure_ascii=False)


def read_direction_results(output_path: Path) -> list[FeedStageResult]:
    """direction 子プロセスの JSON 結果を読む。"""
    if not output_path.exists():
        return []
    with output_path.open("r", encoding="utf-8") as file:
        payload = json.load(file)
    return [
        FeedStageResult.model_validate(item)
...
def direction_result_path(
    ...
    """direction 子プロセスの結果保存先を返す。"""
    return (
        LOG_PATH.parent
        / "distillation_direction_results"
        / f"{case_index:03d}_{case_path.stem}_{sweep_direction}.json"
    )


def direction_timeout_s(candidates: Sequence[int]) -> float:
    """direction 単位の timeout 秒数を返す。"""
    return max(DIRECTION_TIMEOUT_PER_FEED_S, DIRECTION_TIMEOUT_PER_FEED_S * len(candidates))


def run_feed_direction_worker(
    case_path_text: str,
    target_tower: TargetTower,
    sweep_direction: SweepDirection,
    stage_count: int,
    ...
    """子プロセスで一方向の feed 段探索を行う。"""
    case_path = Path(case_path_text)
    output_path = Path(output_path_text)
    results: list[FeedStageResult] = []

    try:
        write_direction_results(output_path, "running", results)
        with hysys_case(case_path.resolve(), visible=VISIBLE_HYSYS) as (
            ...
            log(
                f"[case] file={case_path.name} direction={sweep_direction} "
                f"worker_base_feed_stage={reopened_base_feed_stage}"
            )
            if reopened_stage_count != stage_count:
                raise RuntimeError(
                    f"worker stage_count changed: "
            ...
            for feed_index, feed_stage in enumerate(candidates, start=1):
                log(
                    ...
                    f"direction={sweep_direction} N={stage_count} "
                    f"{feed_index}/{len(candidates)} requested_feed={feed_stage}"
                )
                result = evaluate_feed_stage(
                    simulation_case=simulation_case,
                ...
                if should_stop_direction(result):
                    log(
                        f"[direction-stop] file={case_path.name} "
                        f"direction={sweep_direction} feed={feed_stage} "
                        f"reason={result.invalid_reason}"
                    )
                    break

        write_direction_results(output_path, "ok", results)
    except Exception as exc:
        write_direction_results(output_path, "error", results, str(exc))
        raise


def run_feed_direction_process(
    case_path: Path,
    target_tower: TargetTower,
    sweep_direction: SweepDirection,
    stage_count: int,
    ...
    """一方向の feed 段探索を子プロセスで実行する。"""
    output_path = direction_result_path(case_path, sweep_direction, case_index)
    if output_path.exists():
        output_path.unlink()

    timeout_s = direction_timeout_s(candidates)
    process = mp.Process(
        target=run_feed_direction_worker,
        args=(
    ...
    log(
        f"[direction-process-start] file={case_path.name} "
        f"direction={sweep_direction} timeout_s={timeout_s:.1f}"
    )
    process.start()
    process.join(timeout_s)

    results = read_direction_results(output_path)
    if process.is_alive():
        process.terminate()
        ...
        attempted_feeds = {result.feed_stage for result in results}
        timeout_feed = next(
            (feed_stage for feed_stage in candidates if feed_stage not in attempted_feeds),
            candidates[-1] if candidates else 0,
        )
        timeout_result = FeedStageResult(
            case_name=case_path.name,
            sweep_direction=sweep_direction,
            stage_count=stage_count,
            feed_stage=timeout_feed,
            valid=False,
            invalid_reason=(
                f"direction timeout after {timeout_s:.1f} s "
                f"({DIRECTION_TIMEOUT_PER_FEED_S:.1f} s/feed)"
            ),
        )
        results.append(timeout_result)
        write_direction_results(output_path, "timeout", results, timeout_result.invalid_reason)
        log(
            f"[direction-timeout] file={case_path.name} "
            f"direction={sweep_direction} feed={timeout_feed} "
    ...
    if process.exitcode not in (0, None):
        log(
            f"[direction-process-error] file={case_path.name} "
            f"direction={sweep_direction} exitcode={process.exitcode}"
        )
