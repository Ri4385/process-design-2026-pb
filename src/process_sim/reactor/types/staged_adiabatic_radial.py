...
from process_sim.reactor.types.radial_adiabatic import (
...
class StagedAdiabaticRadialFlowModel:
    ...
    def run(
        self, feed: ReactorFeed, conditions: RadialReactorRunConditions
    ) -> ReactorResult:
        """固定高さの多段ラジアル反応器を計算する。"""
        ...
        for stage_index, (stage_temperature_k, bed_thickness_m) in enumerate(
            zip(
                conditions.stage_inlet_temperatures_k,
                conditions.bed_thicknesses_m,
                strict=True,
            ),
            start=1,
        ):
            ...
            current_pressure_pa = (
                stage_result.outlet.pressure_kpa * self.universal.pa_per_kpa
            )
            if stage_index < len(conditions.stage_inlet_temperatures_k):
                ...
                reheat_pressure_drop_kpa = (
                    conditions.interstage_reheater_pressure_drop_pa
                    / self.universal.pa_per_kpa
                )
                raw_reheat_outlet_pressure_pa = (
                    current_pressure_pa
                    - conditions.interstage_reheater_pressure_drop_pa
                )
                interstage_pressure_positive_values.append(
                    raw_reheat_outlet_pressure_pa > 0.0
                )
        ...
        carbon_error, hydrogen_error = atom_balance_errors(
            feed=feed, outlet=outlet_state.stream
        )
        reactor_pressure_drop_kpa = sum(
            log.reactor_pressure_drop_kpa or 0.0 for log in stage_logs
        )
        reheat_pressure_drop_kpa = sum(
            log.reheat_pressure_drop_kpa or 0.0 for log in stage_logs
        )
        max_re = max(
            (log.max_re_over_one_minus_void or 0.0 for log in stage_logs), default=0.0
        )
        radial_bed_outlet_velocity_ok = all(
            log.outlet_superficial_velocity_m_per_s
        ...
        result_log = ReactorRunLog(
            ...
            * (first_stage.bed_height_m or 0.0),
            ...
            reheat_pressure_drop_kpa=reheat_pressure_drop_kpa,
            total_pressure_drop_kpa=reactor_pressure_drop_kpa
            + reheat_pressure_drop_kpa,
            total_catalyst_volume_m3=sum(
                log.catalyst_volume_m3 or 0.0 for log in stage_logs
            ),
            total_catalyst_mass_kg=sum(
                log.catalyst_mass_kg or 0.0 for log in stage_logs
            ),
            ...
            outlet_pressure_ok=outlet_state.pressure_kpa
            >= conditions.min_outlet_pressure_kpa_abs,
            pressure_positive_ok=all(
                log.pressure_positive_ok is not False for log in stage_logs
            )
        ...
        return ReactorResult(
            outlet=outlet_state,
            eb_conversion=single_reactor.eb_conversion(
                feed=feed, stream=outlet_state.stream
            ),
            styrene_selectivity=single_reactor.styrene_selectivity(
                feed=feed, stream=outlet_state.stream
            ),
            log=result_log,
        )
    ...
    def _validate_conditions(self, conditions: RadialReactorRunConditions) -> None:
        """入力条件の構造を検証する。"""
        if len(conditions.stage_inlet_temperatures_k) != len(
            conditions.bed_thicknesses_m
        ):
            raise ValueError(
                "stage_inlet_temperatures_k and bed_thicknesses_m must have the same length"
            )
        ...
        if any(
            bed_thickness_m <= 0.0 for bed_thickness_m in conditions.bed_thicknesses_m
        ):
