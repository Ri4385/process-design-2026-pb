...
from process_sim.plant.cost.equipment import (
    cooler_outlet_temperature_for_cost,
    selected_reboiler_steam_temperature_c,
)
...
def evaluate_utility_cost(
    ...
    steam_130 = CostBreakdownItem(
        name="steam 130C",
        yen_per_year=annual_energy_value_yen(
            steam_130_duty, UTILITY_PRICE["steam_130c_yen_per_mj"]
        ),
    ...
    steam_160 = CostBreakdownItem(
        name="steam 160C",
        yen_per_year=annual_energy_value_yen(
            steam_160_duty, UTILITY_PRICE["steam_160c_yen_per_mj"]
        ),
    ...
    steam_250 = CostBreakdownItem(
        name="steam 250C",
        yen_per_year=annual_energy_value_yen(
            steam_250_duty, UTILITY_PRICE["steam_250c_yen_per_mj"]
        ),
    ...
    propylene = CostBreakdownItem(
        ...
        yen_per_year=annual_energy_value_yen(
            propylene_duty, UTILITY_PRICE["propylene_yen_per_mj"]
        ),
        duty_kw=propylene_duty,
    )
    electricity = CostBreakdownItem(
        name="electricity",
        yen_per_year=electricity_kw
        * UTILITY_PRICE["electricity_yen_per_kwh"]
        * HOURS_PER_YEAR,
        duty_kw=electricity_kw,
    )
    hexane = CostBreakdownItem(
        name="hexane fuel",
        yen_per_year=annual_mass_value_yen(
            hexane_kg_h, UTILITY_PRICE["hexane_yen_per_kg"]
        ),
        duty_kw=hexane_kg_h,
...
def selected_heater_steam_name(heater: Heater) -> str:
    ...
    maximum_process_temperature = max(
        heater.inlet_temperature_c, heater.outlet_temperature_c
    )
...
def column_condenser_uses_cooling_water(top_temperature_c: float) -> bool:
    """蒸留塔 condenser が冷却水で成立するか判定する。"""
    return top_temperature_c - COOLING_WATER_OUTLET_C >= 10.0
...
def cooling_water_cost_yen_per_year(duty_kw: float) -> float:
    ...
    return (
        cooling_water_ton_h
        * UTILITY_PRICE["cooling_water_yen_per_ton"]
        * HOURS_PER_YEAR
    )
...
def reactor_interstage_reheat_duty_kw(reactor_result: ReactorResult) -> float:
    """反応器段間再加熱 duty を合計する。"""
    return sum(
        (stage.reheat_duty_mw or 0.0) * 1000.0
...
def offgas_combustion_heat_mj_h(plant_record: PlantRunRecord) -> float:
    ...
    return (
        offgas.hydrogen * LHV_MJ_PER_KMOL["hydrogen"]
        + offgas.methane * LHV_MJ_PER_KMOL["methane"]
    )
