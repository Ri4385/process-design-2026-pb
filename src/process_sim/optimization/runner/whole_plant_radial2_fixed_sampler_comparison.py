"""radial 2段固定条件で TPE sampler と GP sampler を比較する runner。"""
...
SamplerKind = Literal["tpe", "gp"]
TrialAttrValue = str | int | float | bool | None

STORAGE_PATH = Path("data") / "optuna" / "whole_plant_radial2_fixed_sampler_comparison.db"
LOG_DIR = Path("logs")
LOG_PATH = LOG_DIR / "whole_plant_radial2_fixed_sampler_comparison.log"
DETAIL_LOG_PATH = LOG_DIR / "whole_plant_radial2_fixed_sampler_comparison_detail.log"
LOG_MAX_BYTES = 10 * 1024 * 1024
LOG_BACKUP_COUNT = 5
SEED = 42
N_STARTUP_TRIALS = 20
TARGET_EFFECTIVE_TRIALS = 100

FIXED_STEAM_TO_EB_RATIO = 5.0
FIXED_DECANTER_1_TEMPERATURE_C = 55.0
FIXED_SM_COLUMN_REFLUX_RATIO = 6.312

TWO_STAGE_RADIAL_FIXED_BED_THICKNESS_RANGE_M = ParameterRange(
    lower=0.6,
    upper=1.0,
)
TWO_STAGE_RADIAL_FIXED_PRESSURE_RANGE_KPA_ABS = ParameterRange(
    lower=80.0,
    upper=120.0,
)
TWO_STAGE_RADIAL_FIXED_CONFIG = replace(
    TWO_STAGE_WHOLE_PLANT_V2_RADIAL_REACTOR_PARAMETER_CONFIG,
    inlet_pressure_kpa_abs=TWO_STAGE_RADIAL_FIXED_PRESSURE_RANGE_KPA_ABS,
    bed_thicknesses_m=(
        TWO_STAGE_RADIAL_FIXED_BED_THICKNESS_RANGE_M,
        TWO_STAGE_RADIAL_FIXED_BED_THICKNESS_RANGE_M,
    ),
)
...
STUDY_CONFIGS: tuple[StudyConfig, ...] = (
    StudyConfig(
        study_name="radial_2stage_fixed_tpe_sampler",
        sampler_kind="tpe",
    ),
    StudyConfig(
        study_name="radial_2stage_fixed_gp_sampler",
        sampler_kind="gp",
    ),
)


def tune_whole_plant_radial2_fixed_sampler_comparison_main() -> None:
    """TPE と GP の sampler 比較 study を累積目標 trial 数まで進める。"""
    configure_logging()
    ...
    for config in STUDY_CONFIGS:
        study = create_or_load_study(config=config, storage_url=storage_url)
        run_study(study=study, config=config, target_trial_count=TARGET_EFFECTIVE_TRIALS)
...
def create_or_load_study(config: StudyConfig, storage_url: str) -> optuna.Study:
    """sampler 種別に応じた単目的 maximize study を作成または読み込む。"""
    return optuna.create_study(
        study_name=config.study_name,
        direction="maximize",
        sampler=create_sampler(kind=config.sampler_kind),
        storage=storage_url,
        load_if_exists=True,
    )


def create_sampler(kind: SamplerKind) -> optuna.samplers.BaseSampler:
    """sampler 種別から Optuna sampler を作る。"""
    if kind == "tpe":
...
def run_hysys_prewarm_with_timeout(case_path: Path, timeout_seconds: float) -> None:
    ...
    logger.info("[prewarm finished] case_path=%s", case_path)
...
def run_study(
    ...
    """1つの sampler study を指定した累積 trial 数まで進める。"""
    ...
    logger.info(
        "[start] study=%s sampler=%s add_trials=%s effective_trials=%s "
    ...
    study.optimize(lambda trial: objective(trial=trial, config=config), n_trials=added_trial_count)
...
def objective(trial: optuna.Trial, config: StudyConfig) -> float:
    ...
    candidate = suggest_radial2_candidate(
        trial=trial,
        parameter_config=TWO_STAGE_RADIAL_FIXED_CONFIG,
    )
    save_sampler_attrs(trial=trial, config=config)
    save_candidate_attrs(trial=trial, candidate=candidate)
    save_separator_candidate_attrs(trial=trial, candidate=FIXED_SEPARATOR_CANDIDATE)
    ...
    logger.info(
        "[trial started] study=%s trial=%s sampler=%s %s %s",
        config.study_name,
        trial.number,
        config.sampler_kind,
        format_candidate(candidate),
        format_separator_candidate(FIXED_SEPARATOR_CANDIDATE),
    )
    result = run_process_with_timeout(
    ...
    if isinstance(result, TrialWorkerFailure):
        trial.set_user_attr("prune_reason", result.reason)
        save_user_attrs(trial=trial, attrs=result.user_attrs)
        logger.info(
            "[pruned] study=%s trial=%s reason=%s attrs=%s",
            config.study_name,
            trial.number,
            result.reason,
            format_key_result_attrs(result.user_attrs),
        )
        raise optuna.TrialPruned(result.reason)
...
def _trial_worker_entry(
    ...
    try:
        ...
        logger.exception("[trial worker failed] %s", format_candidate(candidate))
        result_queue.put(TrialWorkerFailure(reason=str(exc)))
        return
...
def suggest_radial2_candidate(
    trial: optuna.Trial,
    parameter_config: RadialReactorParameterConfig,
) -> RadialReactorCandidate:
    ...
    return RadialReactorCandidate(
        stage_inlet_temperatures_c=(
            suggest_float(
                ...
                parameter_config.stage_inlet_temperatures_c[0],
            ),
            suggest_float(
                trial,
                "stage_2_temperature_c",
                parameter_config.stage_inlet_temperatures_c[1],
            ),
        ),
        inlet_pressure_kpa_abs=suggest_float(
            trial,
            "inlet_pressure_kpa_abs",
            parameter_config.inlet_pressure_kpa_abs,
        ),
        steam_to_eb_ratio=FIXED_STEAM_TO_EB_RATIO,
        bed_thicknesses_m=tuple(
            ...
            for index, parameter_range in enumerate(
                parameter_config.bed_thicknesses_m,
                start=1,
            )
        ),
    )
...
def save_sampler_attrs(trial: optuna.Trial, config: StudyConfig) -> None:
    """sampler 比較用の属性を保存する。"""
    trial.set_user_attr("study_name", config.study_name)
    trial.set_user_attr("sampler_kind", config.sampler_kind)


def save_candidate_attrs(trial: optuna.Trial, candidate: RadialReactorCandidate) -> None:
    """候補条件を trial 属性へ保存する。"""
...
def save_fixed_condition_attrs(trial: optuna.Trial) -> None:
    ...
    trial.set_user_attr("fixed_sm_column_reflux_ratio", FIXED_SM_COLUMN_REFLUX_RATIO)
...
def result_attrs(
    ...
    for stage_log in reactor_result.log.stage_logs:
        ...
        attrs[f"{prefix}_inlet_velocity_m_per_s"] = stage_log.inlet_superficial_velocity_m_per_s
        attrs[f"{prefix}_outlet_velocity_m_per_s"] = stage_log.outlet_superficial_velocity_m_per_s
...
if __name__ == "__main__":
    tune_whole_plant_radial2_fixed_sampler_comparison_main()
