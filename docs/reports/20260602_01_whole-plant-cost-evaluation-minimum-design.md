...
recycle EB と recycle H2O は原料費に入れない。
...
SM は `sm_product` stream 全体の年間販売量上限を `200,000 ton/year` とする。SM product には Styrene 以外の成分も含まれうるため、上限判定は Styrene 成分量ではなく product stream 全体の質量で行う。生産量が上限を超えても、超過分は販売できない。

```text
sm_sales_ton_per_year
  = min(sm_product_stream_ton_per_year, 200_000)
```
...
SM 生産量が `200,000 ton/year` 未満の場合も、計算は止めない。
...
ヘキサン費は utility cost の燃料費として扱い、`hexane_fuel_heat` から求める。余剰オフガスの販売価値は考えない。
...
既存の `plant/economics.py` には暫定的な経済計算があるが、第一段階の全体コスト計算では基本的に使わない。価格、装置費、utility、燃料、収支は `plant/cost/` に改めて定義する。
...
第一段階のログは、Summary と Detail に分ける。Summary は費目ごとの全体像、Detail は入力値や機器ごとの計算結果を確認するためのものとする。

金額表示の基本単位は `億円/year` とする。装置費は、個別に見積もる機器費合計を direct equipment capital とし、その `1.0 倍` を貯槽、バルブ、配管、ポンプ、電気・計装、建屋などのその他設備費として加える。工事費を含めたプラント建設総費用は、direct equipment capital とその他設備費の合計を `2.5 倍` して計算する。その総建設費を 7 年定額償却して、年換算装置費とする。

その他設備費は、プラント建設総費用を求める前の費用項目として扱う。`2.5 倍` は direct equipment capital とその他設備費の合計に掛ける建設費係数であり、0.5 倍の独立した費目は置かない。

人件費は `4 億円/year` の固定費として扱う。保全費はプラント総建設費の `3 %/year` とする。ここでいうプラント総建設費は、`docs/cost.md` の「工事費を含めたプラント建設総費用」であり、direct equipment capital とその他設備費の合計を `2.5 倍` した値を指す。

ログ例は次の形を目安とする。実際の桁幅は実装時に調整する。
...
revenue
  SM      0.XXXX   sales=200000 ton/year, produced=20XXXX ton/year
  BZ      0.XXXX
  TL      0.XXXX
  total   0.XXXX
...
capital
  direct equipment     0.XXXX
  ancillary facilities 0.XXXX
  plant capital factor x2.5
  total plant capital  0.XXXX

annualized equipment
  reactor              0.XXXX
  distillation columns 0.XXXX
  heat exchangers      0.XXXX
  decanters            0.XXXX
  pumps                0.XXXX
  compressors          0.XXXX
  ancillary facilities 0.XXXX
...
utility
  ...
  steam 160C     0.XXXX
  steam 250C     0.XXXX
  cooling water  0.XXXX
  propylene ref  0.XXXX
  electricity    0.XXXX
  hexane fuel    0.XXXX
  total          0.XXXX
...
profit
  revenue - raw_material - equipment - utility - fixed_cost = 0.XXXX
```
...
[Streams]
fresh EB, fresh H2O, SM/BZ/TL product, offgas H2/CH4

[Heat Recovery]
C-11 -> H-22
  C-11 duty, H-22 duty, recovered duty, residual cooling, residual H2O heating
  C-11 Tin/Tout, H-22 Tin/Tout, LMTD, area, annualized cost

[T-Q Streams: no heat recovery]
  HYSYS heater/cooler、蒸留塔 condenser/reboiler、反応器段間再加熱をそのまま process hot/cold stream として出す。
  utility stream は混ぜない。

[T-Q Streams: with heat recovery]
  C-11 と H-22 は、熱回収区間と外部 utility 残り区間に分けて出す。
  その他の heater/cooler、蒸留塔 condenser/reboiler、反応器段間再加熱は no heat recovery と同じ温度範囲で出す。

[External Utility Loads: with heat recovery]
  熱回収後に外部 utility が担当する duty を、T-Q process stream とは別表で出す。
  utility は cooling water、propylene、steam 130C、steam 160C、steam 250C、furnace に分ける。
  furnace は utility 側の温度を持たないため、温度は n/a とする。

[External Utility Summary: with heat recovery]
  T-Q 線図用に、utility 種類ごとの duty 合計も出す。
  cooling water は 30 ℃ -> 45 ℃、propylene は 0 ℃ 一定、steam は各 steam 温度一定、furnace は n/a とする。

[Heat Exchangers]
id                         duty[kW]  Tin  Tout  utility       area[m2]  capital[億円]  annual[億円/year]
decanter_1_cooler1         ...
decanter_1_cooler2         ...
decanter_2_cooler          ...
sm_product_cooler          ...
benzene_product_cooler     ...
toluene_product_cooler     ...
steam_inlet_heater1        ...
steam_inlet_heater2        ...
steam_inlet_heater3        ...
eb_inlet_heater1           ...
eb_inlet_heater2           ...
reactor_trim_heater        ...
T-1 condenser/reboiler     ...
T-2 condenser/reboiler     ...
T-3 condenser/reboiler     ...

[Equipment]
reactor stages
  stage  D[m]  H[m]  catalyst[m3]  reheat[kW]  capital[億円]  annual[億円/year]

distillation columns
  id          D[m]  H[m]  stages  feed  shell[億円]  condenser[億円]  reboiler[億円]  annual[億円/year]
  sm_column   ...
  eb_column   ...
  bztl_column ...

decanters
  id          radius[m]  length[m]  volume[m3]  capital[億円]  annual[億円/year]
  decanter_1  ...
  decanter_2  ...

pumps
  id                       power[kW]  capital[億円]  annual[億円/year]
  water_inlet_pump         ...
  eb_inlet_pump            ...
  sm_column_outlet_pump    ...
  sm_product_pump          ...
...
construction summary
  direct equipment capital[億円]
  ancillary facilities capital[億円] = direct equipment capital * 1.0
  total plant capital[億円] = (direct equipment capital + ancillary facilities capital) * 2.5
  annualized plant capital[億円/year]
  maintenance cost[億円/year] = total plant capital * 0.03

[Warnings]
purity warning, SM below target warning, missing optional values
```
