...
金額表示の基本単位は `億円/year` とする。装置費は、`docs/cost.md` の①から⑥までの機器費合計を base equipment capital とし、工事費を含めたプラント建設総費用をその `2.5 倍` として計算する。その総建設費を 7 年定額償却して、年換算装置費とする。

`docs/cost.md` ⑦の貯槽、バルブ、配管、ポンプ、電気・計装、建屋などの一括費用は、①から⑥までの機器費合計の `1.0 倍` である。ただし、プラント建設総費用を `2.5 倍` として扱う場合、この ⑦ は総建設費の内訳として扱い、別途加算しない。人件費など、`docs/cost.md` に明示されていない運転固定費は含めない。

人件費は `4 億円/year` の固定費として扱う。保全費はプラント総建設費の `3 %/year` とする。ここでいうプラント総建設費は、`docs/cost.md` の「工事費を含めたプラント建設総費用」であり、①から⑥までの機器費合計の `2.5 倍` を指す。
...
construction summary
  base equipment capital[億円]
  ancillary facilities capital[億円] = base equipment capital * 1.0
  total plant capital[億円] = base equipment capital * 2.5
