...
圧力の内部単位は Pa とする。Leite et al. の式を参照する場合でも、実装では SI 単位へ整理してから使い、bar への変換を計算の中心に置かない。ログ表示や設計範囲の説明では、必要に応じて kPa または bar へ換算する。
...
- 1段ケースでは `RadialAdiabaticReactor` を 1 回だけ呼ぶため、1基のみの感度解析にも同じ計算ロジックを使える。
...
# 反応器列出口圧力下限 20 kPa abs (0.2 bar abs)
min_outlet_pressure_pa = 20_000.0
...
5. 最終段以外では、間接加熱により次段入口温度まで再加熱したとみなし、再加熱負荷を計算する。
