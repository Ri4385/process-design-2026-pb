...
段間再加熱では、熱交換器または加熱器を通過することによる圧力損失を考慮する。厳密な伝熱面積や流路設計から圧力損失を計算することは初期設計として重いため、段間再加熱 1 回あたり固定で $20\ \mathrm{kPa}$ の圧力損失を与える。2 段反応器では段間再加熱が 1 回なので合計 $20\ \mathrm{kPa}$、3 段反応器では段間再加熱が 2 回なので合計 $40\ \mathrm{kPa}$ の圧力損失となる。

### 運動量収支

圧力は状態変数として積分する。各点の局所圧力から各成分の分圧を計算し、反応速度評価へ渡す。

Ergun 式は、Leite et al. 2021 の係数と適用範囲を参考にする。ただし、実装では Leite et al. の触媒質量基準式をそのまま移植せず、SI 単位の Ergun 式として組み直して半径方向に積分する。

圧力の内部単位は Pa とする。bar への換算はログ表示や設計範囲の説明に限り、計算の中心には置かない。

半径方向の圧力勾配は、SI 単位で整理した Ergun 式から直接 $dP/dr$ として計算する。Leite et al. 2021 では、ガス密度を $\rho_g$、superficial mass flow を $G$ として扱う。本設計では、ガス密度は局所温度、局所圧力、局所組成から理想気体として計算し、$G$ は局所流通断面積で割った質量速度として扱う。

$$
\rho_g(r) = \frac{P(r)\overline{M}(r)}{R T(r)}
$$

$$
\overline{M}(r)=\sum_j y_j(r)M_j
$$

$$
G(r)=\frac{\dot{m}(r)}{A_r(r)}
=\frac{\dot{m}(r)}{2\pi r z}
$$

Leite et al. 2021 の本文に $\rho_g$ の具体的な計算式が明示されているわけではないが、分圧と熱物性を理想気体ベースで扱っているため、この解釈は同論文の扱いと整合する。外向きラジアルフローでは半径方向に $A_r(r)$ が増加するため、同じ質量流量であれば $G$、線速、Re、圧力損失勾配は外側ほど小さくなる。
...
| 段間再加熱器圧力損失 | $\Delta P_{\mathrm{reheat}}$ | $20\ \mathrm{kPa/stage}$ | 厳密な熱交換器圧損計算は行わず、再加熱 1 回あたり固定値として扱う。 |
| 熱損失 | - | なし | 断熱段として扱う。 |
...
| 項目 | 記号 | 範囲 | 備考 |
|---|---:|---:|---|
| 反応器段数 | $N$ | $2, 3$ | 主設計では $2$ または $3$ 段のみを許容する。1 基のみの計算は感度解析用に `RadialAdiabaticReactor` を直接使う。 |
| 各段入口温度 | $T_{\mathrm{in},i}$ | $590–650\ ^\circ\mathrm{C}$ | コンテスト資料と現行最適化メモに合わせる。段ごとに独立変数とする。 |
| 反応器列入口圧力 | $P_{\mathrm{in}}$ | $50–200\ \mathrm{kPa\ abs}$ | 段間再加熱器圧力損失を考慮すると従来範囲では狭いため、$0.5–2.0\ \mathrm{bar\ abs}$ に広げる。 |
| Steam/EB 比 | $S/EB$ | $5–11$ | 5 未満は炭素析出リスクから外す。Leite et al. の $11$ も比較対象に含める。 |
...
| 項目 | 制約 |
|---|---:|
| 反応器列出口圧力 | $P_{\mathrm{out}} \ge 30\ \mathrm{kPa\ abs}$ |
| 圧力正値 | 全積分点で $P > 0$ |
...
| 元素収支 | C および H の入口出口収支誤差が許容値内であること |
...
src/process_sim/reactor/
  ...
  types/
    radial_adiabatic.py         # 1基の断熱ラジアルフロー反応器
...
docs/
  pfr.md                        # 現行 PFR 実装の恒久メモ
  radial-flow-reactor.md        # ラジアルフロー反応器の恒久メモ
```
...
- 圧力損失計算に必要な単位換算をこの module 内に閉じる。

`src/process_sim/reactor/types/radial_adiabatic.py`

- `RadialAdiabaticReactor` を定義する。
- 1基分の断熱ラジアルフロー固定床反応器を計算する。
- 入力 stream、入口温度、入口圧力、1基分の幾何、Ergun 条件を受け取る。
- 半径方向に積分し、出口 stream、出口温度、出口圧力、圧力損失、1基分のログを返す。
- 感度解析で 1 基のみを評価する場合は、このクラスを直接使えるようにする。

`src/process_sim/reactor/types/staged_adiabatic_radial.py`

- `StagedAdiabaticRadialFlowModel` を定義する。
- `RadialAdiabaticReactor` を段数分だけ呼び出し、多段反応器列を計算する。
- 段間の間接再加熱、全体転化率、全体選択率、各段ログの集約を担当する。
- 多段モデルでは 2 段または 3 段のみを許容する。1 基のみの感度解析は `RadialAdiabaticReactor` を直接使う。
...
- $P_{\mathrm{out}} \ge 30\ \mathrm{kPa\ abs}$、$\mathrm{Re}/(1 - \varepsilon_b) < 500$、圧力正値などの制約値を追加する。

`docs/pfr.md`

- 現行の PFR 実装を命名変更して残す。
- ラジアルフロー反応器とは別の恒久メモとして、PFR の入出力、ログ、制約を管理する。

`docs/radial-flow-reactor.md`

- 実装後に、ラジアルフロー反応器の現行実装範囲を記録する。
- 未実装の予定と実装済み機能を混同しないように分けて記録する。

### 単位系

反応器の新規実装では、反応速度式由来の単位を除き、計算内部は SI 単位で統一する。

| 量 | 内部単位 | 備考 |
|---|---:|---|
| 長さ | m | 半径、高さ、触媒層厚み、粒子径を含む。 |
| 面積 | m2 | $A_r(r)$ など。 |
| 体積 | m3 | 触媒体積など。 |
| 質量 | kg | 触媒質量、質量流量など。 |
| 時間 | s | 収支式、速度式との整合に使う。 |
| 温度 | K | 入力表示は degC でも、計算内部では K とする。 |
| 圧力 | Pa | Ergun 式、密度計算、局所分圧計算で使う。 |
| 物質量流量 | kmol/s | ログ表示では kmol/h に戻す。 |
| 粘度 | Pa s | $\mu = 4.0 \times 10^{-5}\ \mathrm{Pa\,s}$ を使う。 |

反応速度式は既存実装の定義に従う。速度式側が Pa や kmol/(m3 s) を前提にしている場合は、その単位に合わせて渡す。

コードでは、変数名に単位を含める。温度や圧力のように入力表示単位と内部計算単位が異なる値は、変換点を明示する。

```python
# 100 degC (373.15 K)
temperature_k = 373.15

# 反応器列出口圧力下限 30 kPa abs (0.3 bar abs)
min_outlet_pressure_pa = 30_000.0

# 触媒粒子径 3 mm (0.003 m)
pellet_diameter_m = 0.003

# ログ表示用に kmol/s から kmol/h へ戻す。
total_molar_flow_kmol_h = total_molar_flow_kmol_s * 3600.0
```

定数を置く場合も、値だけを置かず、何の単位かをコメントで残す。
...
既存の `ReactorRunConditions` は PFR 用として残す。ラジアルフローでは圧力を状態変数として扱い、内部単位を SI に揃えるため、`pressure_kpa` ではなく `inlet_pressure_pa` を使う。温度も計算条件では `stage_inlet_temperatures_k` として持つ。

### `RadialAdiabaticReactor`
...
`RadialAdiabaticReactor` は 1 基分のラジアル反応器だけを担当する。段間再加熱、次段入口温度への調整、複数段のログ集約は行わない。

主な処理は次の通りである。

1. 入口 stream、入口温度、入口圧力を状態変数に変換する。
2. $x = 0$ から $x = \delta$ まで RK4 で半径方向に積分する。
3. 各積分点で $r = r_0 + x$、$A_r(r)$、局所圧力、局所分圧を計算する。
4. 既存の反応速度式と熱物性計算から $dF/dr$、$dT/dr$ を計算する。
5. Ergun 式から $dP/dr$ を計算する。
6. 出口 stream、出口温度、出口圧力、圧力損失、線速、Re 判定、1基分のログを返す。
...
3. 各段について `RadialAdiabaticReactor.run()` を呼ぶ。
4. 段出口状態、圧力損失、温度低下、転化率、選択率、線速、Re 判定をログ化する。
5. 最終段以外では、間接加熱により次段入口温度まで再加熱したとみなし、再加熱負荷を計算する。同時に、段間再加熱器の圧力損失として $20\ \mathrm{kPa}$ を差し引き、次段入口圧力に反映する。
6. 全段の結果から、全体転化率、全体選択率、反応器列出口状態を計算する。

実装上は $2$ 段、$3$ 段のみを許容する。$1$ 基のみの計算は多段モデルでは扱わず、感度解析用に `RadialAdiabaticReactor` を直接使う。
...
[Overall]
  ...
  outlet P     : 42.410 kPa abs
  reactor pressure drop : 18.915 kPa
  reheat pressure drop  : 40.000 kPa
  total pressure drop   : 58.915 kPa
  EB conversion: 54.23 %
...
[Stage Summary]
  ...
  inlet P [kPa abs]            101.325        74.820         48.110
  outlet P [kPa abs]           94.820         68.110         42.410
  reactor pressure drop [kPa]  6.505          6.710          5.700
...
| 入口圧力 | $50–200\ \mathrm{kPa\ abs}$ |
| Steam/EB 比 | $5–11$ |
| 各段触媒層厚み | $0.3–1.2\ \mathrm{m}$ |
| 触媒床内半径 | 固定 $1.0\ \mathrm{m}$ |
| 触媒床高さ | 固定 $5.0\ \mathrm{m}$ |

Leite et al. 2021 では、最適化結果として後段ほど入口温度が高くなる傾向が示されている。そのため、本設計でも各段入口温度は独立変数として扱い、全段同一温度に固定しない。$1$ 段ケースは多段モデルの設計変数には含めず、必要な場合は `RadialAdiabaticReactor` による単体感度解析として扱う。
...
| 圧損 | なし | Ergun 式 |
| 段間再加熱器圧損 | なし | 再加熱 1 回あたり $20\ \mathrm{kPa}$ |
| 制約 | 探索範囲中心 | 出口圧力、圧力正値、Re 範囲 |
| ログ | 温度、流量、転化率、選択率 | PFR ログに圧力、圧損、線速、Re、触媒量、元素収支を追加 |
...
Leite et al. 2021 では、圧力損失式中の $\rho_g$ をガス密度、$G$ を superficial mass flow として扱っている。単位はそれぞれ $\mathrm{kg/m^3}$、$\mathrm{kg/(m^2\,s)}$ である。

本設計では、ガス密度は局所の温度、圧力、組成から理想気体として計算する。混合平均分子量は $\overline{M}(r)=\sum_j y_j(r)M_j$ とする。これは Leite et al. 2021 に密度式として明記されているものではないが、同論文が分圧と熱物性を理想気体ベースで扱っていることと整合する。

質量速度 $G$ は、局所質量流量をラジアル流れの局所流通断面積で割って求める。すなわち $G(r)=\dot{m}(r)/(2\pi rz)$ とする。反応により総モル流量と組成は変化するため、$\dot{m}(r)$ は各積分点の成分モル流量と分子量から計算する。
...
段間再加熱では熱交換器または加熱器を通過するため、本来は圧力損失が発生する。これを無視すると、低圧運転を評価する本設計では出口圧力を過大評価する可能性がある。一方で、伝熱面積、流路径、流速、熱交換器形式まで含めて厳密に計算するには、反応器設計を超えた詳細設計が必要になる。

したがって、初期実装では再加熱 1 回あたり $20\ \mathrm{kPa}$ の圧力損失を固定値として与える。2 段では $20\ \mathrm{kPa}$、3 段では $40\ \mathrm{kPa}$ を反応器内圧損とは別に差し引く。

### 元素収支ログを出す理由

ラジアルフロー実装では、既存の 6 反応モデルを維持しつつ、積分方向、局所圧力、圧力損失、段間圧損を追加する。そのため、実装上の単位変換や化学量論係数の扱いに誤りがあると、反応前後の元素収支にずれが出る可能性がある。

初期実装では、入口と出口の C および H の総原子流量を比較し、相対誤差をログに出す。これは設計値ではなく、計算整合性の確認用指標として扱う。

### 出口圧力下限を $30\ \mathrm{kPa\ abs}$ とする理由

Leite et al. 2021 では出口圧力制約として $0.5\ \mathrm{bar}$ が用いられている。一方、本設計ではコンテスト資料の低圧運転範囲を参考にしつつ、$0.3\ \mathrm{bar}$ でも相当に低圧であると判断する。したがって、反応器列出口圧力の下限は $P_{\mathrm{out}} \ge 30\ \mathrm{kPa\ abs}$ とする。

### 入口圧力探索範囲を $50–200\ \mathrm{kPa\ abs}$ とする理由

段間再加熱器の圧力損失を考慮しない場合は、入口圧力範囲を $30–150\ \mathrm{kPa\ abs}$ としていた。しかし、3 段反応器では段間再加熱だけで $40\ \mathrm{kPa}$ の圧力損失が加わるため、従来の上限では探索範囲が狭い。したがって、入口圧力の探索範囲は $0.5–2.0\ \mathrm{bar\ abs}$、すなわち $50–200\ \mathrm{kPa\ abs}$ とする。
