...
分離器最適化と一般的なヒートインテグレーションはまだ実装対象外である。HYSYS 接続後の全体コスト評価は `src/process_sim/plant/cost/` に第一段階の実装があり、optimization module にはまだ接続していない。

## 現在の実装範囲

現在の実装範囲は、反応器まわりの最適化入力を表す型と初期値の定義に限定する。
全体コスト評価は `plant/cost/` 側に置き、optimization module からはまだ直接呼ばない。
...
| 各段入口空塔速度 | Pareto v2 では 2.0 m/s 固定 | 2.0 m/s 固定 |
